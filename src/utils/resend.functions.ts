...
async function depositPayUrl(leadId: string, clinicId?: string): Promise<string> {
  ...
  try {
    ...
    if (data?.deposit_token) {
      ...
      if (clinicId) baseUrl.searchParams.set("c", clinicId);
...
export const sendDepositSmsToPatient = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      // Log to sms_messages so the inbox shows the deposit-link thread
      try {
        const admin = getAdminClient();
