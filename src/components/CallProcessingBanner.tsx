    ...
    >
      ...
      <div className="flex flex-col">
        ...
        {!slow && !isDone && !isFail && elapsed > 0 && (
          <span className="text-xs text-foreground">{elapsed}s elapsed</span>
        )}
      </div>
      <button
        onClick={onDismiss}
        className="ml-2 rounded-full p-1 text-foreground hover:bg-surface-soft hover:text-foreground"
        aria-label="Dismiss"
      >
        <X size={14} />
      </button>
