...
export const listClinicPacks = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const db = await assertAdmin(context.claims as Record<string, unknown>);
    const [{ data: packs, error: e1 }, { data: clinics, error: e2 }] = await Promise.all([
      db
        .from("clinic_packs")
        .select("id, clinic_id, pack_size, purchased_at, status, notes, pack_name, amount_paid_ex_gst, date_paid, pack_type")
        .order("purchased_at", { ascending: false }),
      db.from("partner_clinics").select("id, clinic_name, city").order("clinic_name"),
    ]);
    ...
    return {
      packs: (packs ?? []).map((p) => ({
        ...p,
        amount_paid_ex_gst: p.amount_paid_ex_gst == null ? null : Number(p.amount_paid_ex_gst),
      })) as ClinicPackRow[],
      clinics: (clinics ?? []) as ClinicOption[],
    };
  });
...
export const upsertClinicPack = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: inserted, error } = await db
      .from("clinic_packs")
      .insert([{ ...row, purchased_at: new Date().toISOString(), status: "active" }])
      .select("id")
      .single();
    if (error) throw new Error(error.message);
    return { ok: true, id: inserted?.id ?? null };
